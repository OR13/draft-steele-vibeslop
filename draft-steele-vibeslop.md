...
informative:
  ...
  SHIBBOLETH:
    title: "Shibboleth"
    target: https://en.wikipedia.org/wiki/Shibboleth
    author:
      - org: Wikipedia
    date: false
  CLAUDE-OPUS:
...
## A Shibboleth for Agent-Generated Content
...
what creates the impersonation risk described in {{impersonation}}.  A
...
written by the human whose channel it shares.

The marker works best when it is distinctive enough that a human is
unlikely to produce it by accident.  One team member's Agent, for example,
carries a rule of this form in its persistent Context:
...
like the Agent's human is itself a cue to look more closely.

This pattern is a deliberate Shibboleth {{SHIBBOLETH}}.  Historically a
...
The limits of the technique follow from the same analogy, and reinforce
the guidance of {{impersonation}}.  A shibboleth enforced only by
...
cooperating participants, but MUST NOT be relied upon as proof of
authorship; the platform controls described in {{impersonation}} --
